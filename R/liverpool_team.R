...
lvp_team_ls_raw <- get_gym_tables(folder_path = "../pdf/liverpool_team")

lvp_m_final_tb_raw <- lvp_team_ls_raw[[1]] %>% 
  list_rbind() %>% 
  select(!c(1,10)) # 删掉第一列团体排名，最后一列团体总分

lvp_w_final_tb_raw <- lvp_team_ls_raw[[2]] %>% 
  list_rbind() %>% 
  select(!c(1,8))
...
# 创建一个序列，标记每11行为一个组
group <- as.factor((seq_len(nrow(lvp_w_final_tb_raw)) - 1) %/% 11)

# 使用split函数根据组拆分数据框
lvp_w_final_ls_country <- split(lvp_w_final_tb_raw, group) %>% 
  map(  ~{
    # 获取第一行第一列的值
    value <- .x[1, 1]
    # 以“ - ”为分隔符分割字符串，并保留第一部分
...
# 把列表里每个数据框命名为国家名
names(lvp_w_final_ls_country) <- sapply(lvp_w_final_ls_country, function(df) {
  return(df[1, 1]) # 直接返回第二列第一行的值
})

# 删掉每个数据框第一行：团体分数行
lvp_w_final_ls_country <- map(lvp_w_final_ls_country, ~ .x[-1, ])


w_final_ls <- list()

for (j in 1:length(lvp_w_final_ls_country)) {
  page <- lvp_w_final_ls_country[[j]]
  noc <- names(lvp_w_final_ls_country)[j]
...
# 创建一个序列，标记每11行为一个组
group <- as.factor((seq_len(nrow(lvp_m_final_tb_raw)) - 1) %/% 11)

# 使用split函数根据组拆分数据框
lvp_m_final_ls_country <- split(lvp_m_final_tb_raw, group) %>% 
  map(  ~{
    # 获取第一行第一列的值
    value <- .x[1, 1]
    # 以“ - ”为分隔符分割字符串，并保留第一部分
...
# 把列表里每个数据框命名为国家名
names(lvp_m_final_ls_country) <- sapply(lvp_m_final_ls_country, function(df) {
  return(df[1, 1]) # 直接返回第二列第一行的值
})

# 删掉每个数据框第一行：团体分数行
lvp_m_final_ls_country <- map(lvp_m_final_ls_country, ~ .x[-1, ])


m_final_ls <- list()

for (j in 1:length(lvp_m_final_ls_country)) {
  page <- lvp_m_final_ls_country[[j]]
  noc <- names(lvp_m_final_ls_country)[j]
  for (k in seq(1, nrow(page), 2)) {
    Name <- page[k,1]
    FX_d_score <- page[k,3]
    PH_d_score <- page[k,4]
...
lvp_team_final_tb <- bind_rows(lvp_m_final_tb, lvp_w_final_tb) %>% 
  ...
  mutate(Competition = "2022 51st FIG Artistic Gymnastics World Championships", 
         Location = "Liverpool, England",
         Date = "29 Oct 2022 - 6 Nov 2022") %>% # 加上赛事信息
  relocate(LastName, FirstName, Gender, Country, Date, Competition, Round, 
           Location, Apparatus, Rank, D_Score, E_Score, Penalty, Score ) %>% 
  select(!Name:Pen)

write_csv(lvp_team_final_tb, "../cleandata/data_new/liverpool_team.csv")
